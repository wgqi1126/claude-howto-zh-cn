...
| 类型 | 范围 | 共享对象 | 权限方 | 示例 |
|------|------|----------|--------|------|
| Official | Global | 所有用户 | Anthropic | PR Review, Security Guidance |
| Community | Public | 所有用户 | 社区 | DevOps, Data Science |
| Organization | Internal | 团队成员 | 公司 | 内部规范、工具 |
| Personal | Individual | 单用户 | 开发者 | 自定义工作流 |
...
├── commands/             # 以 Markdown 形式存放的 Skills
│   ├── task-1.md
│   ├── task-2.md
│   └── workflows/
├── agents/               # 自定义 agent 定义
...
Plugins 可包含 Language Server Protocol（LSP）支持，以提供实时代码智能。LSP 服务器会在你工作时提供诊断、代码导航与符号信息。
...
官方 Marketplace 提供预配置的 LSP Plugins：

| Plugin | 语言 | 服务器可执行文件 | 安装命令 |
|--------|------|------------------|----------|
...
Plugins 可在清单中通过 `userConfig` 声明可由用户配置的选项。标记为 `sensitive: true` 的值会存入系统钥匙串，而非明文设置文件：
...
Plugins 可通过环境变量 `${CLAUDE_PLUGIN_DATA}` 访问持久化状态目录。该目录按插件区分、在会话之间保留，适合缓存、数据库及其他持久状态：
...
Plugins 可在设置文件中以内联方式定义为 Marketplace 条目，并使用 `source: 'settings'` 字段。这样可直接嵌入插件定义，而无需单独的仓库或 Marketplace：
...
Plugins 可附带 `settings.json` 以提供默认配置。当前支持 `agent` 键，用于设置插件的主线程 agent：
...
| 方案 | 命令名称 | 配置方式 | 最适用场景 |
|------|----------|----------|------------|
| **Standalone** | `/hello` | 在 CLAUDE.md 中手动设置 | 个人、项目专用 |
| **Plugins** | `/plugin-name:hello` | 通过 plugin.json 自动化 | 分享、分发、团队使用 |

快速个人工作流请用**独立斜杠命令**。若要打包多种能力、与团队分享或对外发布分发，请使用 **Plugins**。
...
Anthropic 官方维护的插件目录为 `anthropics/claude-plugins-official`。企业管理员也可以创建私有 Plugin Marketplace，用于内部分发。
...
企业与高级用户可通过设置控制 Marketplace 行为：

| 设置项 | 说明 |
|--------|------|
| `extraKnownMarketplaces` | 在默认之外添加额外的 Marketplace 来源 |
| `strictKnownMarketplaces` | 控制允许用户添加哪些 Marketplace |
| `deniedPlugins` | 由管理员维护的阻止列表，禁止安装指定 Plugins |

<a id="additional-marketplace-features"></a>
### Marketplace 其他特性

- **默认 git 超时**：针对大型插件仓库，由 30 秒提高到 120 秒
- **自定义 npm registry**：Plugins 可指定自定义 npm registry URL 以解析依赖
- **版本固定**：将 Plugins 锁定到特定版本，以获得可复现的环境

<a id="marketplace-definition-schema"></a>
### Marketplace 定义模式（schema）

Plugin Marketplace 在 `.claude-plugin/marketplace.json` 中定义：
...
| `owner` | 是 | 维护该 Marketplace 的组织或用户 |
| `plugins` | 是 | 插件条目数组 |
...
**官方 Marketplace 投稿**：可向 Anthropic 策展的 Marketplace 提交 Plugins，以扩大分发范围。

<a id="strict-mode"></a>
### Strict 模式

控制 Marketplace 定义如何与本地 `plugin.json` 文件协同：

| 设置 | 行为 |
|------|------|
| `strict: true`（默认） | 本地 `plugin.json` 为准；Marketplace 条目对其补充 |
| `strict: false` | Marketplace 条目即完整的插件定义 |

使用 `strictKnownMarketplaces` 时的**组织限制**：

| 取值 | 效果 |
|------|------|
| 未设置 | 无限制 — 用户可添加任意 Marketplace |
| 空数组 `[]` | 锁定 — 不允许任何 Marketplace |
| 模式数组 | 允许列表 — 仅匹配的 Marketplace 可被添加 |
...
> **注意**：在启用 `strictKnownMarketplaces` 的 strict 模式下，用户只能安装来自允许列表中 Marketplace 的 Plugins。适合需要受控插件分发的企业环境。
...
| 使用场景 | 建议 | 原因 |
|----------|------|------|
| **团队入职 / 上手** | ✅ 使用 Plugin | 即时完成设置与全部配置 |
| **框架脚手架** | ✅ 使用 Plugin | 打包框架专用命令 |
| **企业规范** | ✅ 使用 Plugin | 集中分发、版本控制 |
| **快速任务自动化** | ❌ 使用 Command | Plugin 过于复杂 |
...
开发过程中 Plugins 支持热重载。修改插件文件后，Claude Code 可自动检测变更。也可强制重载：
...
## 面向 Plugins 的托管设置（Managed Settings）
...
| `deniedPlugins` | 禁止安装的 Plugins 阻止列表 |
| `extraKnownMarketplaces` | 在默认之外添加额外的 Marketplace 来源 |
| `strictKnownMarketplaces` | 限制用户可添加的 Marketplace |
| `allowedChannelPlugins` | 按发布渠道控制允许使用的 Plugins |
...
这样可确保 Plugins 无法提升权限，或在声明范围之外修改宿主环境。
...
以下 Claude Code 功能可与 Plugins 配合使用：

- **[斜杠命令](../01-slash-commands/)** — 打包在 Plugins 中的独立命令
- **[Memory](../02-memory/)** — 面向 Plugins 的持久化上下文
- **[Skills](../03-skills/)** — 可封装进 Plugins 的领域能力
- **[Subagents](../04-subagents/)** — 作为插件组件包含的专业 agent
- **[MCP 服务器](../05-mcp/)** — 打包在 Plugins 中的 Model Context Protocol 集成
