...
pre-commit hooks 会在每次提交前，本地先运行与 CI 对应的核心检查。提交 PR 前请确保这些检查均能通过。

**必备依赖：**

```bash
# Python 工具链（本项目使用 uv 作为包管理器）
pip install uv
uv venv
...
| `cross-references` | 相对链接、锚点与代码围栏 |
| `mermaid-syntax` | 所有 ` ```mermaid ` 代码块是否可解析 |
| `link-check` | 外部链接是否可访问 |
| `build-epub` | 文档变更时 EPUB 是否可正常生成 |
...
示例：
```
新增(slash-commands): 添加 API 文档生成器
文档(memory): 完善个人偏好示例
修复(README): 修正目录锚点链接
维护(.github): 更新文档检查流程
```
